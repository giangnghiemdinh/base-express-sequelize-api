...
const envVarsSchema = Joi.object()
  .keys({
    ...
    DB_HOST: Joi.string().required().default('localhost').description('DB host'),
    DB_USER: Joi.string().required().description('DB user'),
    DB_PASSWORD: Joi.string().required().description('DB password'),
    DB_POST: Joi.string().required().description('DB post'),
...
module.exports = {
  ...
  database: {
    ...
    options: {
      host: envVars.DB_HOST + (envVars.NODE_ENV === 'test' ? '-test' : ''),
      post: envVars.DB_POST,
